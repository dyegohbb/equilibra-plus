...
export const transactions = pgTable("transactions", {
  id: uuid("id").defaultRandom().primaryKey(), userId: text("user_id").notNull(), walletId: uuid("wallet_id").notNull().references(() => wallets.id), purchaseId: uuid("purchase_id").references(() => purchases.id), scheduledEntryId: uuid("scheduled_entry_id").references(() => scheduledEntries.id), transferId: uuid("transfer_id"), description: text("description").notNull(), amountCents: bigint("amount_cents", { mode: "number" }).notNull(), type: transactionType("type").notNull(), categoryId: uuid("category_id").references(() => categories.id), consumptionDate: date("consumption_date").notNull(), competence: date("competence").notNull(), installmentNumber: integer("installment_number"), installmentTotal: integer("installment_total"), ...audit,
}, (t) => [index("transactions_user_comp_idx").on(t.userId, t.competence), index("transactions_user_wallet_comp_idx").on(t.userId, t.walletId, t.competence), index("transactions_user_consumption_idx").on(t.userId, t.consumptionDate), index("transactions_purchase_idx").on(t.purchaseId), index("transactions_transfer_idx").on(t.transferId), check("transaction_non_zero", sql`${t.amountCents} <> 0`), check("transaction_installments", sql`(${t.installmentNumber} IS NULL AND ${t.installmentTotal} IS NULL) OR (${t.installmentNumber} BETWEEN 1 AND ${t.installmentTotal})`)]);
